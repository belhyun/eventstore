...
$(document).on('click', '.more_load', function(event){
  event.preventDefault();
  var curType = getParamByName('type');
  if(!flag_r && curType == 'rank'){
    page = 2;
    gTotalCnt = 0;
    flag_r = true;
    flag_u = false;
  ...
  if(true){//$(window).scrollTop() == $(document).height() - $(window).height()){
    var url = null;
    if(curType == 'urgent'){
      url = "/products.json?type=urgent&page=";
    }else if(curType =='rank'){
      url = "/products.json?type=rank&page=";
    }
    ...
    $.ajax({
      ...
      success: function(json){
        var total_cnt = json.total_cnt;
        gTotalCnt = total_cnt;
        var v = parseInt(total_cnt/10)+1;
        if(total_cnt % 10 == 0){
          v = parseInt(total_cnt/10);
        }
        if(v < page){
          return;
        }
...
map = new daum.maps.Map(document.getElementById('map'), {
    ...
    var icon = new daum.maps.MarkerImage(
        'http://eventstore.co.kr/assets/event_location_bg.png',
